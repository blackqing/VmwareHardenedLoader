...
- Windows 更新后，如果内置配置不支持当前版本Windows，请使用新生成的外置`dyndata.bin` && `dyndata.sig`。
...
- 某些运行于内核模式的软件保护工具（俗称“壳”）可能会以特征码搜索的方式直接在内核内存中定位 `vmloader.sys` (比如，搜索字面量字符串： `"VMWARE"` `"VEN_15AD"`) 